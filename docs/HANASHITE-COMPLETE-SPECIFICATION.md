...
CREATE TABLE users (
  ...
  status INTEGER DEFAULT 1, -- ユーザー権限（下記参照）
...
| status値 | 権限名 | 説明 | 表示色 |
|---------|--------|------|--------|
| 1 | 運営者 | 管理者権限を持つユーザー | 紫 (bg-purple-100) |
| 2 | 編集者 | コンテンツ編集権限を持つユーザー | 青 (bg-blue-100) |
| 3 | 会員 | 一般ユーザー | 緑 (bg-green-100) |
| 4 | AI会員 | AIによって生成されたユーザー | 黄 (bg-yellow-100) |
| 9 | 停止 | アカウント停止中のユーザー | 赤 (bg-red-100) |

**注意:** 
- `is_banned` フラグとは別に、`status=9` で停止状態を管理します。
- **AI自動投稿の投稿者**: AI自動投稿機能では、`status=4` (AI会員) のユーザーからランダムに選択して投稿します。
...
- 通知ページ
- ポイント履歴ページ
- その他すべてのMyPageMenu使用ページ

### 12.4 アバター画像

**デフォルトアバター:**
- パス: `/images/local-avatars/default-avatar.webp`
- スケール: `scale-125`
- オブジェクトフィット: `object-cover`
