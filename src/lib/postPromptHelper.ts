...
5. 自然で読みやすい文章を最優先
6. 「最近」は1記事に1回まで（全体の30%程度に抑える）
7. 具体的なエピソードは残すが、説明の仕方を大幅に変える（時系列を変える、詳細を追加、視点を変える）
8. 感情や心情を丁寧に描写し、共感を呼ぶ表現にする
9. **元の質問にない背景情報や心情、具体例を積極的に追加する**（「受付のアルバイト中」「70歳くらいの」など）
10. 「〜なんです」「〜でしょうか」「〜ました」など柔らかく丁寧な語尾を使う
11. ブログや日記のような親密さと丁寧さを両立させる
12. 元の質問がわからないレベルまで大幅に変更する
13. **質問は具体的で実用的に**（「上に報告すべきでしょうか」など行動を問う形式）
