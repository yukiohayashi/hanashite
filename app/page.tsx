...
export default async function Home({ searchParams }: HomeProps) {
  ...
  if (sortBy === 'statistics') {
    ...
    // クライアント側で除外
    if (data && excludedPostIds.length > 0) {
      ...
      console.log('未投票アンケ - 除外なし、投稿数:', postsData.length);
    }
      ...
      <main className="md:flex md:justify-center mx-auto mt-14 md:mt-[70px] md:pt-0 max-w-7xl">
        ...
        <div className="flex-1 max-w-[760px]">
          <section>
            ...
            {/* 最新2列（オススメ時のみ表示） */}
            {sortBy === 'recommend' && (
              <div className="gap-[1px] grid grid-cols-2 mb-4 w-full" style={{ boxSizing: 'border-box', padding: 0, margin: '0 0 1rem 0', alignItems: 'stretch' }}>
                {posts && posts.slice(1, 3).map((post) => {
                  const imageUrl = (post as any).og_image || (post as any).thumbnail_url;
                  return (
                    <Link key={post.id} href={`/posts/${post.id}`} className="block bg-white hover:shadow-md border border-gray-300 transition-all" style={{ maxWidth: '100%', height: '100%' }}>
    ...
    <div className="mb-4">
      ...
      {categorySections.map(({ category, posts }) => (
        <div key={category.id} className="mb-3">
          <h4 className="mx-2.5 mb-1 font-bold text-sm text-gray-700">{category.name}</h4>
          <div className="space-y-2 px-2">
